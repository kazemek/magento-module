...
class RpPieperCustom_Ratepayelv_Adminhtml_BulkController extends Mage_Adminhtml_Controller_Action
{
    ...
    private function _ratepayMassEvent($type)
    {
        ...
        foreach ($this->getRequest()->getParam('order_id') as $orderId) {
            ...
            if ($this->_isEventPossible($type, $order)) {
                ...
                $msg = Mage::helper('ratepayelv')->__('The order with the number %s can not be '. $type . 'ed.<br/>', $order->getIncrementId());
            }
        }
        ...
        if ($counter > 0) {
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('ratepayelv')->__('Total of %d record(s) were  '. $type . 'ed.', $counter));
        }
    }
