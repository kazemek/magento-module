...
class RpPieperCustom_Ratepayelv_Adminhtml_LogsController extends Mage_Adminhtml_Controller_Action
{
        ...
        if (!is_array($logIds)) {
            ...
            try {
                foreach ($logIds as $logId) {
                    Mage::getModel('ratepayelv/logging')->load($logId)->delete();
                }
                
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('ratepayelv')->__('Total of %d record(s) were deleted.', count($logIds)));
            } catch (Exception $e) {
    ...
    public function massDeleteExtendedAction()
    {
        ...
        if ($days) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('ratepayelv')->__('Total of %d record(s) were deleted.', $count));
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
        }
